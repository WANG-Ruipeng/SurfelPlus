...
#include "autogen/surfel_integrate.comp.h"

void SurfelIntegratePass::setup(const VkDevice& device, const VkPhysicalDevice& physicalDevice, uint32_t familyIndex, nvvk::ResourceAllocator* allocator)
{
	m_device = device;
	m_pAlloc = allocator;
...
void SurfelIntegratePass::destroy()
{
...
void SurfelIntegratePass::create(const VkExtent2D& size, const std::vector<VkDescriptorSetLayout>& extraDescSetsLayout, Scene* _scene)
{
	...
	computePipelineCreateInfo.stage.module = nvvk::createShaderModule(m_device, surfel_integrate_comp, sizeof(surfel_integrate_comp));
...
const std::string SurfelIntegratePass::name()
{
	return "Surfel Integrate Pass";
}